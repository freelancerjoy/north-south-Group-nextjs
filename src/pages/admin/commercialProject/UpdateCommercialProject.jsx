...
const UpdateCommercialProject = () => {
  const navigate = useNavigate();
  const { id } = useParams();
  const { commercialProject, loadCommercialProject, updateCommercialProject, isLoading } = useCommercialProjectStore();
  ...
  useEffect(() => {
    ...
    if (Array.isArray(commercialProject)) {
      currentProject = commercialProject.find((item) => item._id === id);
    } else if (commercialProject && commercialProject._id === id) {
      currentProject = commercialProject;
    }

    if (currentProject) {
      const parsedStats = typeof currentProject.stats === 'string' ? JSON.parse(currentProject.stats) : currentProject.stats;
      const parsedHighlights = typeof currentProject.highlights === 'string' ? JSON.parse(currentProject.highlights) : currentProject.highlights;
      const parsedSpecs = typeof currentProject.specs === 'string' ? JSON.parse(currentProject.specs) : currentProject.specs;
      const parsedBenefits = typeof currentProject.locationBenefits === 'string' ? JSON.parse(currentProject.locationBenefits) : currentProject.locationBenefits;

      setFormData({
        heroTitle: currentProject.heroTitle || "", heroSubtitle: currentProject.heroSubtitle || "", heroDescription: currentProject.heroDescription || "", heroBadge: currentProject.heroBadge || "", heroMarqueeText: currentProject.heroMarqueeText || "",
        overviewTitle: currentProject.overviewTitle || "", overviewDescription: currentProject.overviewDescription || "", overviewStatusBadge: currentProject.overviewStatusBadge || "", overviewStatusLabel: currentProject.overviewStatusLabel || "",
        signatureSubtitle: currentProject.signatureSubtitle || "", signatureTitle: currentProject.signatureTitle || "", signatureDescription: currentProject.signatureDescription || "",
        highlightsTitle: currentProject.highlightsTitle || "", highlightsSubtitle: currentProject.highlightsSubtitle || "",
        architectureTitle: currentProject.architectureTitle || "", architectureDescription: currentProject.architectureDescription || "",
        workspaceTitle: currentProject.workspaceTitle || "", workspaceDescription: currentProject.workspaceDescription || "",
        galleryTitle: currentProject.galleryTitle || "",
        videoTitle: currentProject.videoTitle || "", videoDescription: currentProject.videoDescription || "", videoUrl: currentProject.videoUrl || "",
        specsTitle: currentProject.specsTitle || "",
        locationTitle: currentProject.locationTitle || "", locationDescription: currentProject.locationDescription || "",
        ctaTitle: currentProject.ctaTitle || "", ctaDescription: currentProject.ctaDescription || "",
      });

      if (parsedStats && parsedStats.length > 0) setStats(parsedStats);
      if (parsedHighlights && parsedHighlights.length > 0) setHighlights(parsedHighlights);
      if (parsedSpecs && parsedSpecs.length > 0) setSpecs(parsedSpecs);
      if (parsedBenefits && parsedBenefits.length > 0) setLocationBenefits(parsedBenefits);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await updateCommercialProject(id, data);
      toast.success("Commercial Project Updated Successfully");
      navigate("/adminDashboard/viewCommercialProject");
    } catch (error) {
      toast.error("Failed to update commercial project");
    }
  };
  ...
  return (
    <div className="mx-auto max-w-5xl p-6">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">Update Commercial Project</h1>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className={sectionClass}>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div><label className={labelClass}>Hero Image (Replace)</label><input type="file" name="heroImage" onChange={handleFileChange} className={inputClass} /></div>
          </div>
        </div>
        ...
        <div className={sectionClass}>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div><label className={labelClass}>Overview Image (Replace)</label><input type="file" name="overviewImage" onChange={handleFileChange} className={inputClass} /></div>
        ...
        <div className={sectionClass}>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div><label className={labelClass}>Architecture Image 1 (Replace)</label><input type="file" name="architectureImage1" onChange={handleFileChange} className={inputClass} /></div>
            <div><label className={labelClass}>Architecture Image 2 (Replace)</label><input type="file" name="architectureImage2" onChange={handleFileChange} className={inputClass} /></div>
          </div>
        </div>
        ...
        <div className={sectionClass}>
          ...
          <div>
            <label className={labelClass}>Gallery Images (Add More)</label>
        ...
        <div className={sectionClass}>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div><label className={labelClass}>Video Thumbnail (Replace)</label><input type="file" name="videoThumbnail" onChange={handleFileChange} className={inputClass} /></div>
          </div>
        </div>
        ...
        <div className={sectionClass}>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div><label className={labelClass}>Map Image (Replace)</label><input type="file" name="mapImage" onChange={handleFileChange} className={inputClass} /></div>
        ...
        <button type="submit" disabled={isLoading} className="mb-8 w-full rounded-md bg-sky-600 p-3 font-semibold text-white hover:bg-sky-700 disabled:opacity-50">
          {isLoading ? "Updating..." : "Update Commercial Project"}
        </button>
      </form>
    </div>
  );
...
export default UpdateCommercialProject;
